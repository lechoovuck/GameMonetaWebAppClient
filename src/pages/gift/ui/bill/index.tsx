...
export const Bill: React.FC<BillProps> = () => {
  ...
  const finalPrice = data && typeof bonus === 'number' && bonus < 0
    ? (totalPrice - (bonus < 0 ? Math.abs(bonus) : 0)).toFixed(2)
  ...
  const send = async () => {
    ...
    if (payload.product_id == 1) (window as any).ym(100155769,'reachGoal','STEAM_TOP_UP')
